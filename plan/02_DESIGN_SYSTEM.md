...
- [x] **`FluxItSwipeRow`** — gesture container that reveals a rose-tinted destructive action on swipe. Android: built on `SwipeToDismissBox` with a custom rose-tinted (`accentRose @ 20%`) background; end-to-start only; `confirmValueChange` fires `onDelete` on dismiss (the store owns removal + 5s undo, so the primitive never animates back). iOS: thin wrapper around native `.swipeActions(edge: .trailing) { Button(role: .destructive) … }` (ships in Phase 07 Slice 7). _Compose: `core/core-designsystem/src/androidMain/.../components/FluxItSwipeRow.kt`._ **Backfilled in Phase 07 Slice 2** (deferral confirmed 2026-05-19; first consumer is the dashboard row). Same slice promoted the `FluxItIconRef→ImageVector` / `ColorToken→Color` mappers (`ListIdentity.kt`) to public DS API and added the four missing list-identity accent swatches (`accent.emerald/orange/indigo/sky`) to `tokens.json`.
...
- [x] All checkboxes above are ✅. _Closed 2026-05-19 with an explicit deferral inventory — the remaining unchecked rows are all deferrals tagged to a later phase rather than gaps. Detailed inventory:_
  ...
  - **§5 `FluxItSwipeRow`** — ~~deferred to Phase 07~~ **backfilled in Phase 07 Slice 2** (Android `SwipeToDismissBox`; iOS `.swipeActions` lands Slice 7).
