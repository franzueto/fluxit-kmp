...
- **Context:** Phase 03 §3 row 4 declares `IconNameAdapter` and §3 row 5 declares `ColorTokenAdapter` — both convert a DB-stored `TEXT` column to a typed Kotlin enum on read. The valid icon name set and the valid color token key set are *generated* artifacts owned by `:core:core-designsystem` (ADR-005 for color tokens, ADR-005a for icons). Two architectural questions follow:
  ...
  2. **If yes to enums, who owns the enum and where does it live?** Three real shapes:
     - **(A) Own copy in `:shared:data`.** Hand-authored `enum class IconRef { CART, HOME, ... }` next to the adapter. Manually kept in sync with `:core:core-designsystem`'s icon set; a sanity test in `:build-logic` verifies parity by parsing both file lists. No dependency edge between `:shared:data` and `:core:core-designsystem`.
     - **(B) Depend on `:core:core-designsystem` and consume the generated enum.** Single source of truth. Requires the icon generator (Phase 02's `:core:core-designsystem:generateIcons` task, owned by build-logic) to emit a *commonMain* enum file alongside today's *androidMain* ImageVector file. Today's generator only emits `FluxItIcons.kt` to `androidMain/` because ImageVector is a Compose type — but the icon *names* are platform-agnostic and can live in commonMain without dragging Compose.
     - **(C) Move the enum to a new shared module `:core:core-tokens` that `:shared:data` and `:core:core-designsystem` both depend on.** Cleanest layering but adds a module and a fresh build.gradle.kts for one enum. Premature factoring for a single point of coupling.
  3. **What happens on read if the DB contains a name that's no longer in the enum?** (Icon renamed in v2 without a corresponding migration; data corruption; future-version row reaching past-version code.) The adapter has to decide between throwing, returning `null`, or returning a sentinel.
- **Decision:** **Option (B)** — `:shared:data` declares a build-time dependency on `:core:core-designsystem` and consumes the *generated* enums. Two adapters are in scope, both following the same pattern:
  - `IconNameAdapter` — `enum class FluxItIconRef` (one entry per source SVG under `core/core-designsystem/icons/*.svg`, name = `UPPER_SNAKE_CASE` of the SVG filename) ↔ `String` (the lowercase-hyphenated name used by Compose accessors and iOS asset-catalog imagesets, e.g. `cart` / `cart-filled`). One enum value per icon, including filled variants (`CART`, `CART_FILLED`).
  - `ColorTokenAdapter` — `enum class FluxItColorToken` (one entry per leaf in `tokens.json`'s `dark` group's color section, name = `UPPER_SNAKE_CASE` of the dotted token key, e.g. `PRIMARY_BLUE`, `ACCENT_ROSE`, `BACKGROUND_DARK`) ↔ `String` (the dotted token key as authored, e.g. `primary.blue`, `accent.rose`, `background.dark`).
  - **Module dependency:** `shared/data/build.gradle.kts` adds `implementation(project(":core:core-designsystem"))` in commonMain. The Konsist rule in `:build-logic`'s arch tests is updated to permit this *one* edge with a named exception (`:shared:data` → `:core:core-designsystem`) and forbid every other UI-direction edge from data.
  - **Phase 02 retrofit (small, in-scope for the §3 commit that lands `IconNameAdapter`):** The icon generator (`:core:core-designsystem:generateIcons`, defined in `build-logic`) currently emits `FluxItIcons.kt` into `core-designsystem/build/generated/source/icons/androidMain/` (per ADR-005a). The retrofit adds a second emission: `FluxItIconRef.kt` (pure-Kotlin enum) into `core-designsystem/build/generated/source/icons/commonMain/`. Same single source (the `core/core-designsystem/icons/*.svg` file list) drives both outputs. No new Gradle task — same generator, one extra output file. The androidMain `FluxItIcons.kt` then maps `FluxItIconRef` → `ImageVector` keyed by the enum, instead of duplicating the name set. Token generator (ADR-005) gets the analogous retrofit: emit `FluxItColorToken.kt` into `commonMain/` alongside today's androidMain `FluxItColors.kt`. The Phase 02 `verifyIconsInSync` + `verifyTokensInSync` tasks both cover the new outputs by construction (regenerate → diff).
  - **Adapter shape:** the adapter is the standard SQLDelight `ColumnAdapter<EnumType, String>` two-method pair (`encode(value: EnumType): String` + `decode(databaseValue: String): EnumType`). Encoding is total — every enum maps to its canonical string. Decoding handles the unknown-name case (see next).
  - **Unknown-name fallback policy (decoding):** on `decode("some-unknown-icon")`, the adapter logs a warning via Kermit (`Kermit.w { "Unknown icon name '$value' in DB row; falling back to default" }`) and returns a sentinel: `FluxItIconRef.LIST` for icons, `FluxItColorToken.PRIMARY_BLUE` for color tokens. Reasoning: throwing on read corrupts the Flow consumer (the dashboard would crash on a single bad row); returning `null` would force every read site to handle nullability for a case that the schema invariant says cannot happen; the sentinel preserves the Flow stream and surfaces the bug via logs + Crashlytics breadcrumb (Phase 16 will wire Kermit → Crashlytics). The "right" fix for a missing enum value is always a schema migration (ADR-006: rename `list.icon = 'old-name'` → `list.icon = 'new-name'` in `<N>.sqm`), not a runtime fallback — the sentinel is a safety net, not a strategy.
  - **Acyclicity verified:** the dep edge is `:shared:data` → `:core:core-designsystem` → (nothing it imports back). `:core:core-designsystem` does not depend on `:shared:data`, `:shared:domain`, or any feature/state module. The graph stays a DAG.
  - **Status flip:** stays Proposed until §3's commit lands `IconNameAdapter` + `ColorTokenAdapter` referencing the *generated* commonMain enums (so the Phase 02 retrofit is implicit in the same commit or a precursor) and the updated Konsist rule passes. Pattern matches ADR-006 / 006a / 006b status-flip discipline.
- **Consequences:**
  - ➕ DB rows are referentially safe end-to-end: no string typo can sneak past the encode side; every read goes through a typed enum that exhaustive `when`s can cover at consumer sites.
  - ➕ Single source of truth for both icon names and color tokens. Adding an icon in Phase 02's pipeline auto-grows the enum that `:shared:data`'s adapter consumes — no per-phase manual sync step.
  - ➕ Phase 02 retrofit is a small, contained change (one extra emitter output per generator) that *also* improves the design system itself: today's androidMain `FluxItIcons.kt` accessors will become typed-enum-keyed, which means a future Compose Multiplatform target (a v2 possibility under ADR-001) can move the accessors to commonMain without a name-set duplication step.
  - ➕ Unknown-name fallback is a safety net with a Crashlytics breadcrumb path — bugs surface as logs and incident counts, not as user-visible crashes.
  - ➕ Konsist's "permit exactly this edge, forbid all other UI-direction edges" rule documents the coupling in machine-checkable form.
  - ➖ A real dependency edge from `:shared:data` (deep-stack) to `:core:core-designsystem` (capability-layer leaf, but UI-flavored historically). The diagram in MASTER_PLAN.md shows data sitting *above* core-* in the inward-arrows convention, so the edge is architecturally valid — but a reader scanning the diagram might still flinch. Mitigated by this ADR's existence: when a future reviewer asks "why does data depend on designsystem?", `00_DECISIONS.md` has the answer.
  - ➖ The Phase 02 retrofit (extra emitter output) is small but cross-phase work. Phase 02 is "closed" per the master plan; retroactively touching it requires the cross-phase touch to be called out in the §3 commit's pre-commit summary. Acceptable cost; the alternative (option (A)'s hand-mirrored enum) is worse.
  - ➖ The unknown-name fallback to a fixed sentinel (`FluxItIconRef.LIST`, `FluxItColorToken.PRIMARY_BLUE`) means a user with bad DB data sees a wrong-but-not-broken UI. Better than crashing, worse than impossible. Mitigated by the migration discipline of ADR-006 (renames go through `<N>.sqm`, never as silent data drift) and by the Kermit warning that ensures the bug doesn't go undetected.
  - ➖ Two adapters (icons + color tokens) follow the same pattern — there's a temptation to factor out a generic `EnumColumnAdapter<E : Enum<E>>` helper. Resist for v1; the two cases differ in fallback sentinel and the helper is more abstraction than the two-call-site code earns.
  - 🔁 If a third typed-enum adapter materializes in v2 (e.g. a typography token adapter for a v2 feature that stores per-list custom typography), revisit by extracting `EnumColumnAdapter<E>`. Three cases is the cliché threshold; we're at two for now.
  - 🔁 If at any point `:core:core-designsystem` needs to depend on something `:shared:data` already depends on (creating cycle risk), supersede with an option-(C)-style `:core:core-tokens` ADR.
- **Alternatives considered:**
  - **(A) Hand-mirrored enum in `:shared:data`** — rejected: drift is the default failure mode for parallel hand-authored lists. The "sanity test parses both file lists" check works, but only at CI time; PR authors will discover the failure after the fact rather than designing around the constraint. Single-source-of-truth at the generator level is strictly better.
  - **(C) New `:core:core-tokens` shared module** — rejected for v1: factoring overhead for one point of coupling exceeds the cost of the direct edge. Re-evaluate if a second consumer (e.g. `:shared:state` or a feature module) needs the enums *and* must avoid pulling in the Compose-bearing `:core:core-designsystem`.
  - **String-only adapters (no enums)** — rejected: re-introduces the stringly-typed surface that ADR-006a's id-source ban and the broader Konsist/detekt regime are designed against. Cheap up front, expensive in 18 months.
  - **Throw on decode of unknown name** — rejected: corrupts the Flow consumer for the entire query, including unrelated rows. One bad row brings down the dashboard. Worse than the sentinel fallback under any threat model.
  - **Return `null` on decode of unknown name** — rejected: forces every read site to handle nullability for an invariant the schema says cannot happen. Sites would either ignore the case (bug) or write defensive `?: defaultIcon` everywhere (the same sentinel, just diffused). Centralizing the fallback in the adapter is cleaner.
  - **Auto-migrate "unknown icon name → default" on read** (i.e. the adapter writes back to the DB) — rejected: adapters are pure transforms in SQLDelight's model; writing through them turns a `SELECT` into a side-effect-bearing operation. Schema migrations go through `<N>.sqm` files (ADR-006), not through adapter side effects.
- **Resolves Open Questions in Phase 03:** §3 row 4 (icon adapter coupling) and the implicit "what does ColorTokenAdapter look like?" question that §3 row 5 raises. Implies a Phase 02 retrofit (extra emitter output for both icon and token generators) that §3's commit will land alongside the adapters.

---

### ADR-007 — In-house `Outcome<T, E>` type vs. `kotlin.Result<T>` vs. Arrow `Either`
- **Status:** Accepted (flipped from Proposed on 2026-05-28 at Phase 04 Slice 11A — early, ahead of the originally-planned §13 hand-off, by explicit decision once the preconditions were materially met: every `:shared:domain` repository signature and every command use case returns `Outcome<T, E>` with no `kotlin.Result` / `Throwable` leak at the boundary, the Konsist `kotlin.Result` + `arrow.*` bans in `:shared:domain` shipped Slice 2, and `Outcome.mapError` + `fold` are exercised by the §7 Lists use-case tests. Reactive read use cases return `Flow`, not `Outcome` — a subscription has no single fold-able failure, so the ADR's "typed-error result" scope covers commands, not streams. Remaining §7 use cases inherit the locked decision.)
- **Date:** 2026-05-28
...
- **Status:** Accepted (flipped from Proposed on 2026-05-28 at Phase 04 Slice 11A — early, ahead of the originally-planned §13 hand-off, by explicit decision: four Lists use cases (`ObserveLists`, `SearchLists`, `CreateList`, `RenameList`) landed as classes with constructor-injected dependencies and a single `operator fun invoke` / `suspend operator fun invoke`; their per-use-case commonTest suites wire the §11 repository fakes through the constructor-injection seam (the fakes are the second area exercising the injection contract, complementing the Lists feature area); and the Konsist arch test now enforces "no top-level `suspend fun` in `usecase/`" — implemented by filtering `file.functions()` to `isTopLevel`, so member `invoke` operators are correctly exempt. The originally-stated "three use cases across two *feature* areas" bar was read pragmatically: the shape is locked and the remaining §7 use cases inherit it.)
- **Date:** 2026-05-28
