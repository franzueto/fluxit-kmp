...
**Last updated:** 2026-05-28 (Phase 03 mid-flight; §5/§6/§7/§8 complete + §10 commonTest foundation + integration test; remaining §10 sub-bullets + §13 hand-off ahead)
...
**Phase 03 — Data Layer, §10 (Test pyramid) — foundation + integration landed; remaining sub-bullets ahead.** ✅ §10(g) shipped: `expect fun inMemoryDriver()` in `commonTest` with JVM (JdbcSqliteDriver) + iOS (`NativeSqliteDriver` with `inMemory = true`) actuals; all five smoke tests promoted. ✅ §10(e) shipped: `IntegrationFlowTest` walks the DoD scenario (create → add 3 items → toggle → schedule recurring reminder → close → reopen → assert) backed by a new `PersistentTestDb` expect/actual (file-backed JVM via temp `JdbcSqliteDriver`; file-backed iOS via `NativeSqliteDriver` rooted in `NSTemporaryDirectory()`). 46 tests green on both targets. Remaining §10 work: (a) per-query unit tests in `commonTest` — one per query in each `.sq` file (the smoke tests cover the repo surface, not every query path); (b) mapper round-trip tests for all four mappers; (c) repository error-path tests (Conflict on FK violation, Validation on bad input, Storage on driver IO); (d) migration test harness (zero migrations yet, so the harness itself is the test); (f) concurrency test: 50 concurrent `setCompleted` on the same item never deadlock. Once §10 is green on both targets in CI, §13 hand-off flips ADR-006/006a/006b to Accepted, marks 006c as Superseded by ADR-007a, and advances ▶ Next Step to Phase 04. Phase 02 carry-forward still pending: wire `verifyTokensInSync` + `verifyIconsInSync` into `.github/workflows/ci.yml`; Phase 07 backfills `FluxItSwipeRow` + long-press → ThemeGallery wiring + optional `Font.fluxIt.*` SwiftUI accessor. §10's full test pyramid lands after the four repos; that's also where the iOS NativeSqliteDriver in-memory helper promotes the smoke tests from `androidUnitTest` to `commonTest`. §13 closes the phase: flip ADR-006/006a/006b to Accepted, mark ADR-006c as Superseded by ADR-007a, advance ▶ Next Step to Phase 04. Phase 02 carry-forward items still pending for a future cycle: wire `verifyTokensInSync` + `verifyIconsInSync` into `.github/workflows/ci.yml`; Phase 07 backfills `FluxItSwipeRow` + long-press wiring to ThemeGallery + optional `Font.fluxIt.*` SwiftUI accessor.
