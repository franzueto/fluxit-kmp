...
**Last updated:** 2026-05-28 (Phase 04 §7 Lists use cases complete through the CRUD + appearance wave on Slices 11A/11B of `phase/04-domain-layer`; `ObserveLists` / `SearchLists` / `CreateList` / `RenameList` / `SetListStarred` / `UpdateListAppearance` shipped, ADR-007 + ADR-007b Accepted)
...
**Phase 04 Slice 12 — §7 use-case wave two.** Slices 11A/11B closed the Lists CRUD + appearance wave (`ObserveLists`, `SearchLists`, `CreateList`, `RenameList`, `SetListStarred`, `UpdateListAppearance`) as ADR-007b classes, added the Konsist "no top-level suspend fun in usecase/" rule, and flipped ADR-007 + ADR-007b → Accepted. **Next** lands the following §7 use cases — pick the next coherent slice from: (a) the remaining **Lists** use cases — `ReorderList` (pure `SortOrderArithmetic.between` → `repo.reorder`), `DeleteList` (soft-delete + cancel active reminders, returns a `DeletedListSummary` for undo, cascade per ADR-006b), `UndoDeleteList`; or (b) the **Items** feature area — `ObserveListDetail` (combines Lists + Items flows via `flow.combine`), `AddItem`, `ToggleItemCompleted`, `UpdateItemDetails` (the `Optional<T>` partial-intent shape from §6/§4 first lands here), `ReorderItem`, `SetItemStarred`, `ClearCompletedItems` / `RestoreItems`. `DeleteList` + the Items photo/reminder use cases will pull in the deferred `ReminderScheduler` / `PhotoCapture` / `AnalyticsSink` ports + their fakes (`FakeReminderScheduler`, `RecordingAnalyticsSink`) as they're first consumed. After §7: §9 concurrency-contract review + §13 hand-off (the remaining gate items — branch coverage ≥95%, Phase 04 → 🟢). Phase 02 carry-forward still pending for a future cycle: wire `verifyTokensInSync` + `verifyIconsInSync` into `.github/workflows/ci.yml` (ADR-007a's parity check rides on this); Phase 07 backfills `FluxItSwipeRow` + long-press wiring to ThemeGallery + optional `Font.fluxIt.*` SwiftUI accessor.
...
| 04 | Domain Layer | [`04_DOMAIN_LAYER.md`](plan/04_DOMAIN_LAYER.md) | 🔵 In progress | 66% |
