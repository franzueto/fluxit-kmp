...
**Last updated:** 2026-05-28 (Phase 04 §7 use-case wave three kicked off on Slice 13A of `phase/04-domain-layer`; `ReorderList` shipped as a pure delegate to `ListsRepository.reorder` — the repo owns the `SortOrderArithmetic` math, mirroring `ReorderItem`. Remaining wave-three work in flight: `UpdateItemDetails` + the `Optional<T>` primitive (13B), Reminders use cases + `ReminderScheduler` port + `DeleteList` (13C), Photos + `PhotoJanitor` + `InitializeApp` (13D). `UndoDeleteList` stays blocked on a data-layer restore primitive `ListsRepository` doesn't expose.)
...
| 04 | Domain Layer | [`04_DOMAIN_LAYER.md`](plan/04_DOMAIN_LAYER.md) | 🔵 In progress | 75% |
