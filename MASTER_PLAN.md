...
**Last updated:** 2026-05-28 (Phase 04 §7 use-case wave three in progress on `phase/04-domain-layer`; Slices 13A/13B/13C shipped `ReorderList`, `UpdateItemDetails`+`Optional<T>`, and the Reminders wave — `ReminderScheduler` port + `SchedulerFailure`/`NotInFuture` error variants + `ScheduleReminder`/`CancelReminder`/`RehydrateReminders`/`ObserveRemindersFor{List,Item}` + `DeleteList` (composes `CancelReminder`, returns `DeletedListSummary`). Remaining: Photos + `PhotoJanitor` + `InitializeApp` (13D). `UndoDeleteList` stays blocked on a data-layer restore primitive `ListsRepository` doesn't expose.)
...
| 04 | Domain Layer | [`04_DOMAIN_LAYER.md`](plan/04_DOMAIN_LAYER.md) | 🔵 In progress | 88% |
