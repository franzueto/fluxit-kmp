...
**Last updated:** 2026-05-28 (Phase 04 §7 use-case build-out **complete** on `phase/04-domain-layer`; Slice 13D shipped the Photos wave — `PhotoCapture` port + `CaptureFailure` variant + `AttachPhotoToItem`/`DetachPhotoFromItem`/`PhotoJanitor` (per-photo) + `InitializeApp` composite. Slices 13A/13B/13C earlier shipped `ReorderList`, `UpdateItemDetails`+`Optional<T>`, and the Reminders wave + `DeleteList`. Remaining §7 items are all **data-layer-blocked** (`UndoDeleteList`/`UndoDeleteItem`/`RestoreItems`/`ClearCompletedItems→List<ItemId>` need a restore/`RETURNING` primitive; `CreateList` analytics needs `AnalyticsSink`; batch `PhotoJanitor` needs `selectOrphaned`). Next: §9 concurrency review + §13 hand-off (coverage ≥95%, Phase 04 → 🟢).)
...
**Phase 04 §9 concurrency review + §13 hand-off.** The §7 use-case build-out is **complete** (Slices 11A→13D): all Lists/Items/Reminders/Photos use cases + the four ports (`Clock`/`IdGenerator`/`ReminderScheduler`/`PhotoCapture`) + the six-variant `DomainError` + `Optional<T>` ship with per-use-case tests, all green on JVM + iOS Sim. **Next** closes Phase 04: (a) **§9 concurrency-contract review** — confirm use cases are dispatcher-agnostic, Flows cold/conflated, no `runBlocking`/`GlobalScope`; (b) **§13 hand-off** — verify branch coverage ≥95% (e.g. via Kover), then flip Phase 04 → 🟢 and advance ▶ Next Step to Phase 05 (State Management). **Known deferrals to carry into a later cycle** (all blocked on contracts outside `:shared:domain`): `UndoDeleteList`/`UndoDeleteItem`/`RestoreItems` + `ClearCompletedItems→List<ItemId>` (data-layer restore / `RETURNING id`); `CreateList`'s `AnalyticsEvent.ListCreated` (needs `AnalyticsSink`); batch `PhotoJanitor selectOrphaned(24h)` (data-layer enumeration); §5 ports `AppLogger`/`AnalyticsSink`/`ConfigProvider` (no §7 consumer needed them yet). Phase 02 carry-forward still pending: wire `verifyTokensInSync` + `verifyIconsInSync` into `.github/workflows/ci.yml`; Phase 07 backfills `FluxItSwipeRow` + long-press wiring to ThemeGallery + optional `Font.fluxIt.*` SwiftUI accessor. Original ordering, for reference: (a) **remaining Lists** — `ReorderList` (delegate to `repo.reorder`, ✅ 13A), `DeleteList` (soft-delete + cancel active reminders, returns a `DeletedListSummary` for undo, cascade per ADR-006b — needs the `ReminderScheduler` port), `UndoDeleteList`; (b) **`UpdateItemDetails`** — the use case that introduces the `Optional<T>` "don't-touch vs set-null" primitive (§6/§4), reading the current item then emitting a complete `ItemPatch`; (c) **Reminders** use cases (`ScheduleReminder` / `CancelReminder` / `RehydrateReminders` / `ObserveReminders*`) — pull in the `ReminderScheduler` port + `FakeReminderScheduler`; (d) **Photos / housekeeping** (`AttachPhotoToItem` / `DetachPhotoFromItem` / `PhotoJanitor`) — pull in the `PhotoCapture` port; and the **app-level** `InitializeApp`. Note several deferred items are blocked on contracts the shipped repos don't expose yet — `UndoDeleteItem` / `RestoreItems` + the `ClearCompletedItems → List<ItemId>` variant need a data-layer restore / `RETURNING id` method, and `CreateList`'s analytics emission needs §5's `AnalyticsSink`. After §7: §9 concurrency-contract review + §13 hand-off (branch coverage ≥95%, Phase 04 → 🟢). Phase 02 carry-forward still pending for a future cycle: wire `verifyTokensInSync` + `verifyIconsInSync` into `.github/workflows/ci.yml` (ADR-007a's parity check rides on this); Phase 07 backfills `FluxItSwipeRow` + long-press wiring to ThemeGallery + optional `Font.fluxIt.*` SwiftUI accessor.
...
| 04 | Domain Layer | [`04_DOMAIN_LAYER.md`](plan/04_DOMAIN_LAYER.md) | 🔵 In progress | 93% |
