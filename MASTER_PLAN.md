...
**Last updated:** 2026-05-28 (Phase 04 §8 pure business rules first wave — `CompletionCalculator` + `SortOrderArithmetic` + `PaletteCatalog` — landed on Slice 7 of `phase/04-domain-layer`)
**Architect:** _you_ + Claude (Senior Mobile Architect role)
**Repo phase:** Phase 04 (Domain Layer) **in progress** on branch `phase/04-domain-layer`. Slices 1–6 closed §1–§3 + §5 first wave + §6 error model. Slice 7: §8 pure business rules first wave — `object CompletionCalculator` (fraction/display/isFullyComplete with `require` guards; 0/0 not complete per dashboard intent), `object SortOrderArithmetic` (between/needsCompaction with `SEED_SORT_ORDER = 1.0` + `REBALANCE_EPSILON = 1e-9` constants matching `SqlListsRepository` verbatim — the data layer's inline math is a follow-up migration when a §7 use case first needs pure-code computation), `object PaletteCatalog` (wraps `ColorToken.entries` + `FluxItIconRef.entries` as the single Create-List-picker source). 24 new tests green on JVM + iOS Sim, including the §8 random-inserts property test (1000 inserts via `between()` produce midpoints strictly inside their brackets) and a targeted "tightest-gap inserts trigger compaction" test. `RecurrenceCalculator` deferred to its own slice. Pending list renumbered: the Phase 05 MVI ADR moved from collision-spot "ADR-007" to ADR-014.

---

## ▶ Next Step

**Phase 04 Slice 8 — §8 `RecurrenceCalculator` + `FakeClock` advance-by fixture.** The heavier §8 rule that needs its own slice: `nextFireAfter(rule: RecurrenceRule, after: Instant, tz: TimeZone): Instant?` covering all four `RecurrenceRule` variants (`None` → null; `Daily` → +24h snapped to original wall-clock time with DST handling; `Weekly(days)` → next day-of-week match at original wall-clock time; `Monthly(dayOfMonth)` → same day-of-month next month with Jan 31 → Feb 28/29 → Mar 31 clamping). Property tests across `Europe/London` (DST), `Asia/Kolkata` (no DST, +5:30 offset), and `Pacific/Apia` (DST + dateline shift) per the §8 KDoc. Introduces the reusable `FakeClock(initial: Instant)` fixture with `advanceBy(Duration)` — the §11 fixtures table's first richer fake, lands here because `RecurrenceCalculator` is the first rule that needs time-advance semantics in tests. After Slice 8: §11 fixtures (FakeListsRepository / FakeItemsRepository / etc. — the testing scaffolding §7 use cases will lean on), then §7 use cases proper. Phase 02 carry-forward still pending for a future cycle: wire `verifyTokensInSync` + `verifyIconsInSync` into `.github/workflows/ci.yml` (ADR-007a's parity check rides on this); Phase 07 backfills `FluxItSwipeRow` + long-press wiring to ThemeGallery + optional `Font.fluxIt.*` SwiftUI accessor.
...
| 04 | Domain Layer | [`04_DOMAIN_LAYER.md`](plan/04_DOMAIN_LAYER.md) | 🔵 In progress | 42% |
