...
**Last updated:** 2026-05-28 (Phase 04 §5 platform ports first wave — `Clock` + `IdGenerator` — landed on Slice 5 of `phase/04-domain-layer`)
**Architect:** _you_ + Claude (Senior Mobile Architect role)
**Repo phase:** Phase 04 (Domain Layer) **in progress** on branch `phase/04-domain-layer`. Slices 1–4 closed §1–§3 (modulo source-layout row that fills as later slices populate the directories). Slice 5: §5 platform ports first wave — `Clock` (`fun interface` with `companion object { val System }` binding to `kotlinx.datetime.Clock.System`) and `IdGenerator` (typealias to `:core:core-utils`'s existing port, per ADR-006a) landed under `:shared:domain/port/`; `:shared:domain` build script gained `implementation(project(":core:core-utils"))`; `ListId.Companion.new(idGen)` factory (the §2 deferred row) landed alongside. 4 new tests green on JVM + iOS Sim. Heavier ports (`ReminderScheduler`, `PhotoCapture`, `AppLogger`, `AnalyticsSink`, `ConfigProvider`) wait for the slices that consume them. Pending list renumbered: the Phase 05 MVI ADR moved from collision-spot "ADR-007" to ADR-014.

---

## ▶ Next Step

**Phase 04 Slice 6 — §6 error model build-out (`DomainError` + `DataError.toDomain()` mapper).** §6 already ships `Outcome<T, E>` (Phase 03 §5), `DataError` (Phase 03 §5), and `ValidationError` seed (Slice 3). What's missing: the `sealed class DomainError` sum (`Validation`, `NotFound`, `Conflict`, `StorageFailure`, `SchedulerFailure`, `CaptureFailure`) and the `DataError.toDomain(): DomainError` extension that lets repository `Outcome<T, DataError>` flow through use cases as `Outcome<T, DomainError>` without losing variant info. Per ADR-007's precondition, the mapper is the thing that prevents `DataError` from leaking past the domain edge — landing it here unblocks every §7 use case that returns `Outcome<T, DomainError>`. `SchedulerError` / `CaptureError` variants get placeholder slots; the actual sealed types land with `ReminderScheduler` / `PhotoCapture` ports in later slices. Test coverage: every `DataError` variant maps to the expected `DomainError` variant (exhaustive `when` test). Phase 02 carry-forward still pending for a future cycle: wire `verifyTokensInSync` + `verifyIconsInSync` into `.github/workflows/ci.yml` (ADR-007a's parity check rides on this); Phase 07 backfills `FluxItSwipeRow` + long-press wiring to ThemeGallery + optional `Font.fluxIt.*` SwiftUI accessor.
...
| 04 | Domain Layer | [`04_DOMAIN_LAYER.md`](plan/04_DOMAIN_LAYER.md) | 🔵 In progress | 28% |
