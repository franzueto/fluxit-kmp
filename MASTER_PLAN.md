...
**Last updated:** 2026-05-28 (Phase 04 §11 Lists + Items repository fakes landed on Slice 9 of `phase/04-domain-layer`)
**Architect:** _you_ + Claude (Senior Mobile Architect role)
**Repo phase:** Phase 04 (Domain Layer) **in progress** on branch `phase/04-domain-layer`. Slices 1–8 closed §1–§3 + §5 first wave + §6 + §8 (Completion/SortOrder/Palette/Recurrence). Slice 9: §11 repository fakes wave one — `FakeListsRepository` + `FakeItemsRepository` (MutableStateFlow-backed; tombstone filtering on reads; sort-order minting + per-list compaction via `SortOrderArithmetic`; `NotFound` returns on missing/tombstoned ids). Each fake stays independent: counters NOT computed inside `FakeListsRepository` (use-case tests `flow.combine` with `FakeItemsRepository` for the dashboard projection), cascade NOT implemented (per ADR-006b application-layer). `:shared:domain` commonTest gained `libs.kotlinx.coroutines.test` for `runTest`. 15 new tests green on JVM + iOS Sim. `FakeRemindersRepository` + `FakePhotosRepository` deferred to Slice 10 alongside the use cases that consume them. Pending list renumbered: the Phase 05 MVI ADR moved from collision-spot "ADR-007" to ADR-014.

---

## ▶ Next Step

**Phase 04 Slice 10 — §11 repository fakes wave two (`FakeRemindersRepository` + `FakePhotosRepository`).** Rounds out the §11 fakes inventory ahead of the §7 use-case slices that need them. `FakeRemindersRepository` backs the four `RemindersRepository` methods (observeForOwner, observeUpcoming, schedule, reschedule, cancel, rebindPlatformHandle) with a `MutableStateFlow<List<Row>>`; `observeUpcoming` captures "now" via injected `Clock.now()` at subscription time (matches the §5 spec). `FakePhotosRepository` backs ingest/observe/deleteIfOrphaned with an in-memory map + a parallel "file" map so `ingest`'s file-first contract is observable in tests; `deleteIfOrphaned` takes an optional "items repo" lambda for the live-reference check or accepts a simpler test-only flag. After Slice 10: §7 use-case wave one (Lists — `ObserveLists`, `SearchLists`, `CreateList`, `RenameList`, `SetListStarred` — exercising Slice 5's ports + Slice 6's error mapper + Slice 7's rules + Slices 9/10's fakes). Phase 02 carry-forward still pending for a future cycle: wire `verifyTokensInSync` + `verifyIconsInSync` into `.github/workflows/ci.yml` (ADR-007a's parity check rides on this); Phase 07 backfills `FluxItSwipeRow` + long-press wiring to ThemeGallery + optional `Font.fluxIt.*` SwiftUI accessor.
...
| 04 | Domain Layer | [`04_DOMAIN_LAYER.md`](plan/04_DOMAIN_LAYER.md) | 🔵 In progress | 53% |
