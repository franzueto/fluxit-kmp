...
**Last updated:** 2026-05-28 (Phase 04 §7 Items use-case wave one landed on Slice 12 of `phase/04-domain-layer`; `ObserveListDetail` / `AddItem` / `ToggleItemCompleted` / `SetItemStarred` / `ReorderItem` / `DeleteItem` / `ClearCompletedItems` shipped — the cleanly-mapping subset; UpdateItemDetails, photo, and bulk-undo/restore use cases deferred pending the `Optional<T>` primitive + `PhotoCapture` port + a data-layer restore method)
...
| 04 | Domain Layer | [`04_DOMAIN_LAYER.md`](plan/04_DOMAIN_LAYER.md) | 🔵 In progress | 72% |
