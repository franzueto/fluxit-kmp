...
**Last updated:** 2026-05-28 (Phase 04 §8 `RecurrenceCalculator` + reusable `FakeClock` fixture landed on Slice 8 of `phase/04-domain-layer`)
**Architect:** _you_ + Claude (Senior Mobile Architect role)
**Repo phase:** Phase 04 (Domain Layer) **in progress** on branch `phase/04-domain-layer`. Slices 1–7 closed §1–§3 + §5 first wave + §6 error model + §8 first wave (Completion/SortOrder/Palette). Slice 8: §8 `RecurrenceCalculator` with `nextFireAfter(rule, after, tz)` covering all four `RecurrenceRule` variants via a shared `findNext(after, tz, advance)` core that wall-clock-round-trips candidates to detect DST spring-forward gaps + relies on kotlinx-datetime's default fall-back resolution (earlier-of-ambiguous). Reusable `FakeClock(initial).advanceBy(Duration)` test fixture under `:shared:domain/commonTest/.../port/`. 27 new tests green on JVM + iOS Sim, including three iteration property tests (Daily 365× in London with both DST transitions; Monthly(31) 24× in Apia covering the dateline-shift edge; Weekly 200× in Kolkata's fixed +5:30) asserting strict monotonicity + per-variant invariants. Pending list renumbered: the Phase 05 MVI ADR moved from collision-spot "ADR-007" to ADR-014.

---

## ▶ Next Step

**Phase 04 Slice 9 — §11 repository fakes (`FakeListsRepository`, `FakeItemsRepository`, `FakeRemindersRepository`, `FakePhotosRepository`).** With `FakeClock` and `IdGenerator` in place from Slices 5/8, the next slice rounds out §11's "fakes" inventory with the four in-memory repository implementations §7 use-case tests will lean on. Each repo fake backs reads with a `MutableStateFlow<List<Entity>>` so `observe…` flows behave naturally; writes update the state flow synchronously; cascade semantics (per ADR-006b — ListsRepo.softDelete cascades to LIST-owned reminders; ItemsRepo.softDelete cascades to ITEM-owned reminders) implemented in the fakes so use-case tests against them reflect the real repo behavior the application-level cascade use cases will compose. `FakeIdGenerator(prefix = "id")` and `RecordingAnalyticsSink` (foreshadowing the §5 AnalyticsSink port) bundle in if small enough; otherwise they slot into the §7 use-case slices that first need them. After Slice 9: §7 use-case wave one (Lists — `ObserveLists`, `SearchLists`, `CreateList`, `RenameList`, `SetListStarred` — exercising the fakes + Slice 5's ports + Slice 6's error mapper + Slice 7's rules). Phase 02 carry-forward still pending for a future cycle: wire `verifyTokensInSync` + `verifyIconsInSync` into `.github/workflows/ci.yml` (ADR-007a's parity check rides on this); Phase 07 backfills `FluxItSwipeRow` + long-press wiring to ThemeGallery + optional `Font.fluxIt.*` SwiftUI accessor.
...
| 04 | Domain Layer | [`04_DOMAIN_LAYER.md`](plan/04_DOMAIN_LAYER.md) | 🔵 In progress | 48% |
