...
**Last updated:** 2026-05-28 (Phase 04 §7 use-case wave three in progress on `phase/04-domain-layer`; Slice 13A shipped `ReorderList` (pure delegate, repo owns `SortOrderArithmetic`), Slice 13B shipped `UpdateItemDetails` + the `Optional<T>` "don't-touch vs set-null" primitive (in `error/`). Remaining wave-three work in flight: Reminders use cases + `ReminderScheduler` port + `DeleteList` (13C), Photos + `PhotoJanitor` + `InitializeApp` (13D). `UndoDeleteList` stays blocked on a data-layer restore primitive `ListsRepository` doesn't expose.)
...
| 04 | Domain Layer | [`04_DOMAIN_LAYER.md`](plan/04_DOMAIN_LAYER.md) | 🔵 In progress | 79% |
