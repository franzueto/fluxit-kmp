...
**Last updated:** 2026-05-28 (Phase 03 mid-flight; §5/§6/§7/§8 complete + §10 commonTest foundation + integration + concurrency + mapper + per-query gap tests; remaining §10 sub-bullets c/d + §13 hand-off ahead)
...
**Phase 03 — Data Layer, §10 (Test pyramid) — foundation + integration + concurrency landed; per-query / mapper / error-path / migration harness still ahead.** ✅ §10(g) shipped: `expect fun inMemoryDriver()` in `commonTest` with JVM (JdbcSqliteDriver) + iOS (`NativeSqliteDriver` with `inMemory = true`) actuals. ✅ §10(e) shipped: `IntegrationFlowTest` walks the DoD scenario backed by a `PersistentTestDb` expect/actual (file-backed JVM via temp `JdbcSqliteDriver`; file-backed iOS via `NativeSqliteDriver` rooted in `NSTemporaryDirectory()`). ✅ §10(f) shipped: `SqlItemsRepositoryConcurrencyTest` fires 50 alternating `setCompleted` calls in parallel via `Dispatchers.Default` + `async`/`awaitAll`. ✅ §10(b) shipped: mapper round-trip tests. ✅ §10(a) shipped: per-query gap coverage in `commonTest/.../db/UncoveredQueriesTest.kt` — the 37 queries reached by repo paths stay covered transitively via smoke + integration; the 8 queries no repo currently calls (selectAllActive, updateMetadata, hardDelete, countActive on Lists; countByList on Items; setActive, selectNeedingReschedule on Reminders; selectOrphaned on Photos) get direct `db.<table>Queries.<query>()` tests. 63 tests green on both `:shared:data:testDebugUnitTest` and `:shared:data:iosSimulatorArm64Test`. Remaining §10 work: (c) repository error-path tests (Conflict on FK violation, Validation on bad input, Storage on driver IO); (d) migration test harness (zero migrations yet, so the harness itself is the test). Once §10 is green on both targets in CI, §13 hand-off flips ADR-006/006a/006b to Accepted, marks 006c as Superseded by ADR-007a, and advances ▶ Next Step to Phase 04. Phase 02 carry-forward still pending: wire `verifyTokensInSync` + `verifyIconsInSync` into `.github/workflows/ci.yml`; Phase 07 backfills `FluxItSwipeRow` + long-press → ThemeGallery wiring + optional `Font.fluxIt.*` SwiftUI accessor. §10's full test pyramid lands after the four repos; that's also where the iOS NativeSqliteDriver in-memory helper promotes the smoke tests from `androidUnitTest` to `commonTest`. §13 closes the phase: flip ADR-006/006a/006b to Accepted, mark ADR-006c as Superseded by ADR-007a, advance ▶ Next Step to Phase 04. Phase 02 carry-forward items still pending for a future cycle: wire `verifyTokensInSync` + `verifyIconsInSync` into `.github/workflows/ci.yml`; Phase 07 backfills `FluxItSwipeRow` + long-press wiring to ThemeGallery + optional `Font.fluxIt.*` SwiftUI accessor.
